...
const routine_validation = {
  ...
  check_all_availability: async (data: TRoutine_input, require_all = true) => {
    ...
    let idx = 0;
    if (academic_year_id) {
      if (!results[idx++]) throw new Api_error("Academic year not found", 404);
    }
    if (class_id) {
      if (!results[idx++]) throw new Api_error("Class not found", 404);
    }
    if (section_id) {
      if (!results[idx++]) throw new Api_error("Section not found", 404);
    }
    if (teacher_id) {
      if (!results[idx++]) throw new Api_error("Teacher not found", 404);
    }
    if (subject_id) {
      if (!results[idx++]) throw new Api_error("Subject not found", 404);
    }
